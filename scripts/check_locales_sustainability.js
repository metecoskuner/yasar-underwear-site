...
function checkLocale(l) {
	const p = path.resolve(__dirname, '..', 'src', 'locales', l + '.json');
	let raw;
	try { raw = fs.readFileSync(p, 'utf8'); } catch (e) { void e; return { locale: l, error: 'file-not-found' }; }
	let j;
	try { j = JSON.parse(raw); } catch (e) { void e; return { locale: l, error: 'json-parse-error' }; }
